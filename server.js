...
// MQTT CONFIG
const MQTT_SERVER = "mqtts://c8aa35bd14934f5ebd190c284377a7fe.s1.eu.hivemq.cloud:8883";
...
mqttClient.on("message", async (topic, message) => {
    try {
        ...
        const { data: insertedData, error } = await supabase
            .from("sensor_data")
            .insert([{ temp, gas, temp_alert, gas_alert }])
            .select();
            ...
            try {
                await axios.post(
                    "https://api.onesignal.com/api/v1/notifications",
                    {
                        ...
                        headings: { en: "Smart4 Alert" },
                        contents: {
                            en: temp_alert && gas_alert
                ...
                console.error("❌ OneSignal error:", notificationError.response?.data || notificationError.message);
            }
        }
    } catch (error) {
...
const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && req.url === "/") {
        ...
        res.end(JSON.stringify({ status: "ok", mqtt: mqttClient.connected ? "connected" : "disconnected" }));
        return;
    }
    ...
    if (req.method === "GET" && req.url === "/data") {
        try {
            ...
            console.error("❌ Supabase /data:", error);
